...
impl WebSocketFrame {
    ...
    pub fn write<W: Write>(&self, output: &mut W) -> io::Result<()> {
        ...
        println!("header as u16 = {:?}", hdr);

        match self.header.payload_length {
            PAYLOAD_LEN_U16 => {
                println!(
                    "payload length representable as u16 ({})",
                    self.payload.len()
                );
                try!(output.write_u16::<BigEndian>(self.payload.len() as u16))
            }
            PAYLOAD_LEN_U64 => {
                println!(
                    "payload length representable as u64 ({})",
                    self.payload.len()
                );
                try!(output.write_u64::<BigEndian>(self.payload.len() as u64))
            }
            _ => {
                println!(
                    "payload length less than {} ({})",
                    PAYLOAD_LEN_U16,
                    self.payload.len()
                );
            }
    ...
    pub fn from_close(recv_frame: &WebSocketFrame) -> WebSocketFrame {
